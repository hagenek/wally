...
* [`src/mister_wally/`](src/mister_wally/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/mister_wally/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `mister-wally` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/mister_wally/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns mister-wally.example
  (:require [mister-wally.config :as config])
